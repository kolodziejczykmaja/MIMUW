...
void GetBoard(char ***Board, int *row) {
    ...
    *row = 0;
    ...
    while (flag == 0) {
        int col = 0;
        // Alokujemy miejsce na nowy wskaźnik do wiersza
        char **newBoard = realloc(*Board, (size_t)(*row + 1) * sizeof(char*));
        if (newBoard == NULL) {
            // W przypadku błędu alokacji zwalniamy całą planszę
            if (*Board != NULL) {
                for (int i = 0; i < *row; i++) {
                    free((*Board)[i]);
                }
        ...
        *Board = newBoard;
        ...
        // Wczytujemy znaki do wiersza
        while (symbol != '\n') {
            char *newRow = realloc((*Board)[*row], (size_t)(col + 1) * sizeof(char));
            if (newRow == NULL) {
                // W przypadku błędu zwalniamy całą planszę
                for (int i = 0; i < *row; i++) {
                    free((*Board)[i]);
                }
                free((*Board)[*row]);  // Zwalniamy aktualny, częściowo zaalokowany wiersz
                free(*Board);
                *Board = NULL;
                return;
            }
...
bool Posibility(char **Board, Coordinates boxCoordinates, int row,
	...
	if (direction == UP || direction == DOWN) {
		...
		if (strlen(Board[y - 1]) <= (size_t)x || strlen(Board[y + 1]) <= (size_t)x) {
			return false;
		}
	...
	if (direction == LEFT || direction == RIGHT) {
		...
		if (strlen(Board[y]) <= (size_t)x + 1 || strlen(Board[y]) <= (size_t)x - 1) {
			return false;
		}
	...
	while (symbol != '.') {
		if (symbol == '\n') {
			...
			if (Posibility(*Board, box, row, direction) &&
				...
				//char **newBoard = CopyBoard(*Board, row);
