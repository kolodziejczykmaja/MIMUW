 ...
 * "Sokoban"
 *
 * Program simulates the Sokoban game.
 *
 * The program input consists of an initial board state description
 * and a non-empty sequence of commands terminated by a period.
 * The first command is a board state print command.
 * The program ignores input content after the terminating period.
 * The program reads the initial board state description, then
 * reads and executes subsequent commands.
 * Recognized commands are: print current board state, push a box,
 * and undo a previously executed push.
 * Unlike typical Sokoban implementations, the user does not need
 * to specify moves for the player through empty spaces.
 * The program automatically determines how to move the player to
 * a position from which pushing a box in the indicated direction
 * is possible.
 * The program output is the result of executing board state
 * print commands.
 *
...
// Function to deallocate the board.
...
// Structure storing a stack node.
typedef struct StackNode {
  char **board;
  struct StackNode *next;
} StackNode;

// Structure storing a stack.
...
bool IsStackEmpty(Stack *stack) { return (stack->top == NULL); }
...
// Free memory of boards on stack.
...
// Structure storing a queue node.
typedef struct QueueNode {
  Coordinates coord;
  struct QueueNode *next;
} QueueNode;

// Structure storing a queue.
...
bool IsCellEmpty(char cell) { return (cell == EMPTY || cell == GOAL); }
...
// Check if player can reach position in front of box.
...
bool Posibility(char **Board, Coordinates boxCoordinates, int row,
  ...
  if (direction == UP || direction == DOWN) {
    ...
    if ((!IsAccessible(Board[y + 1][x])) || (!IsAccessible(Board[y - 1][x]))) {
      return false;
    }
  }
  if (direction == LEFT || direction == RIGHT) {
    ...
    if ((!IsAccessible(Board[y][x + 1])) || (!IsAccessible(Board[y][x - 1]))) {
      return false;
    }
  }
  return true;
...
// Change player and box positions.
  ...
  *row = 0;
  ...
  while (flag == 0) {
    ...
    *Board = newBoard;
    (*Board)[*row] = NULL;
    while (symbol != '\n') {
      char *newRow = realloc((*Board)[*row], (size_t)(col + 1) * sizeof(char));
      assert(newRow != NULL);
      (*Board)[*row] = newRow;
